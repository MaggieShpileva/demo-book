...
export const BookPage = forwardRef<HTMLDivElement, BookPageProps>(({ page }, ref) => {
  ...
  return (
    ...
    >
      {page.title != null && (
        <h2 className={isCover ? styles.coverTitle : styles.pageTitle}>{page.title}</h2>
      )}
      ...
      {page.pageNumber != null && <span className={styles.pageNumber}>{page.pageNumber}</span>}
    </article>
  );
});
